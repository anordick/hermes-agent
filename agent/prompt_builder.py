...
KANBAN_GUIDANCE = (
    ...
    "4. **Block on genuine ambiguity or role/authority limits.** "
    "If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "The user will unblock with context and the dispatcher will respawn you.\\n"
    "   - **Ambiguity only** (missing info, unclear instruction): block only — "
    "no handoff card needed.\\n"
    "   - **Outside your role, tools, permissions, confidence, or authority**: "
        "block the original card, THEN create exactly one handoff card assigned to "
        "your supervisor (from $HERMES_SUPERVISOR, or \"vaile\" if unset):\\n"
        "     `kanban_create(title=\"[handoff] blocked: outside my authority\", "
        "assignee=\"<supervisor>\", "
        "body=\"lifecycle: handoff_to_supervisor\\nfrom_profile: <your-profile>\\n"
        "to_profile: <supervisor>\\norg_path: <your-profile> -> <supervisor>\\n"
        "reason: outside authority/role/permissions\\n"
        "blocked_card: <your-task-id>\\n\\n"
        "Bounded description of what was blocked and why\")`\\n"
        "     Use your actual supervisor from the $HERMES_SUPERVISOR environment "
        "variable. Do not hardcode a profile name — the supervisor is set by the "
        "dispatcher at spawn time.\""
    "   - Do not create multiple handoff cards for the same block.\\n"
    "   - **Never escalate directly to Mobius.** Mobius is not a spawnable "
    "Kanban profile. Human escalation reaches Mobius only through Vaile's "
    "Discord surfacing channel, after Vaile reviews and decides.\\n"
    "   - **Escalate one level at a time.** If your supervisor is unresponsive "
    "after 2+ of your runs, escalate to the next level up — never skip levels "
    "directly to the top.\\n"
    "   - **If $HERMES_SUPERVISOR is not set**, block only and "
    "explain the missing supervisor in your block reason.\"\n"
    ...
    "- Do not create cards assigned to Mobius. Mobius is not a spawnable "
    "Kanban profile and has no agent session to receive them.\n"
    "- Do not escalate directly to Mobius. All human escalation passes through "
    "Vaile, who alone may surface items to Mobius via Discord under the "
    "accepted surfacing policy.\n"
    "- Escalate one level at a time through the org hierarchy. If your "
    "supervisor is unresponsive after 2+ of your runs, go to the next "
    "supervisor level — never skip directly to the top."
)
